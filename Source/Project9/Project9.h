...
public:
	...
	static FString GetRoleString(const AActor* InActor)
	{
		...
		if (IsValid(InActor) == true)
		{
			...
			FString RemotRoleString = UEnum::GetValueAsString(TEXT("Engine.ENetRole"), InActor->GetRemoteRole());

			RoleString = FString::Printf(TEXT("%s / %s"), *LocalRoleString, *RemotRoleString);
		}
